...
> **⚠️ WAVE fork of pristine [`hung-yueh/react-native-litert-lm@0.3.6`](https://github.com/hung-yueh/react-native-litert-lm).**
> **One change only:** the upstream wrapper fed a single `maxTokens` into
> *both* `litert_lm_engine_settings_set_max_num_tokens` (engine KV-cache
> budget) and `litert_lm_session_config_set_max_output_tokens` (per-call
> decode cap). For the stock litert-community Gemma 4 E2B bundle these must
> differ (engine ≈ 2048, decode ≤ 256), so no single `maxTokens` value
> could run a ~1846-token prompt. `LLMConfig` now exposes **`engineMaxTokens`**
> (default 2048) and **`outputMaxTokens`** (default 256); `maxTokens` is
> kept as a back-compat fallback for both.
>
> Everything else — the prebuilt framework download, `postinstall`, the
> `cpp/include/litert_lm_engine.h` C header, and the `litertLm` v0.10.2
> pinning — is left **pristine**, so the 0.3.6 C++ bridge still compiles
> against upstream's v0.10.2 prebuilt framework. See WAVE postmortem
> `docs/postmortems/gemma4-litert-stock-limits-research.md` (Path A).
