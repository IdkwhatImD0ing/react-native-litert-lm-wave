...
export interface LLMConfig {
   ...
   *
   * @deprecated WAVE fork: upstream applied this single value to BOTH the
   * engine-wide KV-cache budget and the per-call decode cap, which are two
   * distinct LiteRT-LM knobs. Prefer {@link engineMaxTokens} and
   * {@link outputMaxTokens}. If only `maxTokens` is set it is used as the
   * fallback for both, preserving the old (broken-for-long-prompts)
   * behaviour.
   * @default 1024
   */
  maxTokens?: number;
  ...
  /**
   * Engine-wide KV-cache budget (total input + output tokens), passed to
   * `litert_lm_engine_settings_set_max_num_tokens`. Must fit the bundle's
   * compiled `cache_length` — 2048 for the stock litert-community Gemma 4
   * E2B/E4B `.litertlm` bundle. Setting this above the compiled graph's
   * ceiling causes "failed to invoke the compiled model".
   * @default 2048
   */
  engineMaxTokens?: number;

  /**
   * Per-call decode cap, passed to
   * `litert_lm_session_config_set_max_output_tokens`. Must stay within the
   * bundle's compiled decode chunk — <=256 for the stock Gemma 4 bundle.
   * @default 256
   */
