...
void HybridLiteRTLM::loadModelInternal(
  ...
  if (config.has_value()) {
    ...
    // WAVE fork: decouple the engine KV budget from the per-call decode
    // cap. Explicit knobs win; otherwise fall back to legacy maxTokens so
    // existing callers don't change behaviour, then to the safe defaults.
    if (config->engineMaxTokens.has_value()) {
      engineMaxTokens_ = config->engineMaxTokens.value();
    } else if (config->maxTokens.has_value()) {
      engineMaxTokens_ = config->maxTokens.value();
    }
    if (config->outputMaxTokens.has_value()) {
      outputMaxTokens_ = config->outputMaxTokens.value();
    } else if (config->maxTokens.has_value()) {
      outputMaxTokens_ = config->maxTokens.value();
    }
  ...
  auto tryCreateEngine = [&](const char* backend, const char* visionBackend) -> bool {
    ...
    litert_lm_engine_settings_set_max_num_tokens(settings, static_cast<int>(engineMaxTokens_));
  ...
  if (session_config_) {
    litert_lm_session_config_set_max_output_tokens(session_config_, static_cast<int>(outputMaxTokens_));
