...
private:
  ...
  // WAVE fork: upstream fed maxTokens_ into BOTH the engine-wide KV-cache
  // budget AND the per-call decode cap, which are two distinct LiteRT-LM C
  // API knobs. They are now decoupled:
  //  - engineMaxTokens_ -> litert_lm_engine_settings_set_max_num_tokens
  //      (total input+output budget; must fit the bundle's compiled
  //       cache_length — 2048 for the stock litert-community Gemma 4
  //       E2B/E4B bundle)
  //  - outputMaxTokens_ -> litert_lm_session_config_set_max_output_tokens
  //      (per-call decode cap; <=256 for the stock bundle's compiled
  //       decode chunk)
  double engineMaxTokens_ = 2048.0;
  double outputMaxTokens_ = 256.0;
